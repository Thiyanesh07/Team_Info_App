...
class _TeamChatScreenState extends ConsumerState<TeamChatScreen> {
  ...
  Future<void> _initialSetup() async {
    ...
    _typingSub = _socket.typingStream.listen((data) {
      if (data['userId'] != ref.read(authProvider).user?.id &&
  ...
  void _handleSend() {
    ...
    _socket.sendTeamMessage(text, replyToId: replyToId);
  ...
  void _setReplyMessage(TeamMessage message) {
    setState(() => _replyingTo = message);
  }

  String _messagePreviewText(TeamMessage message) {
  ...
  Future<void> _togglePin(TeamMessage msg) async {
    final res = await _api.put('${ApiConstants.teamChat}/${msg.id}/pin');
    if (!mounted) return;
            ...
            child: _loading
                ...
                : ListView.builder(
                    ...
                    itemBuilder: (_, i) {
                      ...
                      final isMe =
                          msg.senderId ==
                          user?.id; // In TeamMessage, we use senderId
              ...
              ConstrainedBox(
                ...
                child: ListView.builder(
                  ...
                  itemBuilder: (ctx, i) => ListTile(
                    ...
                    title: Text(
                      pinned[i].message ?? 'Image',
                      style: const TextStyle(color: Colors.white, fontSize: 14),
                    ),
                    subtitle: Text(
                      pinned[i].sender?['name'] ?? '',
