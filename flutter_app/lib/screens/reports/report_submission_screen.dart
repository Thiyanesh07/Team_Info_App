...
class _ReportSubmissionScreenState extends ConsumerState<ReportSubmissionScreen> {
  ...
  Future<void> _pickFile() async {
    ...
    final List<String> extensions = widget.request.allowedFormats.map((e) => e.replaceAll('.', '')).toList();
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      if (_selectedFile != null) {
        final uploadRes = await api.uploadImage(_selectedFile!.path!); // Shared endpoint handles auto resource_type
  ...
  Future<void> _deleteSubmission() async {
    final ok = await showDialog<bool>(
      ...
      builder: (context) => AlertDialog(
        ...
        title: const Text('Delete Submission?', style: TextStyle(color: Colors.white)),
        content: const Text('Are you sure you want to delete your report submission?', style: TextStyle(color: AppColors.textSecondary)),
        actions: [
    ...
    if (ok == true) {
      setState(() => _submitting = true);
      try {
        final response = await ref.read(appDataRepositoryProvider).deleteReportSubmission(widget.submission!.id);
        if (response.success) {
          if (mounted) Navigator.pop(context, true);
        }
      } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        title: Text('Report Submission', style: GoogleFonts.outfit(fontWeight: FontWeight.bold)),
               ...
               SizedBox(
                 ...
                 child: ElevatedButton(
                   ...
                   child: _submitting 
                     ...
                     : Text(widget.submission == null ? 'Submit Report' : 'Update Submission', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                 ),
               ),
            ],
          ],
     ...
     return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          _buildDetailRow(Icons.file_copy_outlined, 'Formats', widget.request.allowedFormats.join(', ')),
          
          if (widget.submission?.status == ReportSubmissionStatus.REDO && widget.submission?.reviewerNotes != null) ...[
                ...
                children: [
                  const Row(
                    children: [
                      ...
                      SizedBox(width: 8),
                      Text('REVISION REQUESTED', style: TextStyle(color: AppColors.error, fontWeight: FontWeight.bold, fontSize: 11)),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Text(widget.submission!.reviewerNotes!, style: const TextStyle(color: Colors.white, fontSize: 13)),
                ],
              ),
            ),
          ],
      ...
      child: Container(
        ...
        child: Column(
          children: [
            ...
            Text(
              _selectedFile?.name ?? (_uploadUrl != null ? 'Update existing file' : 'Tap to select document'),
            ...
            if (_selectedFile != null) ...[
               ...
               Text('${(_selectedFile!.size / 1024).toStringAsFixed(1)} KB', style: const TextStyle(color: AppColors.textMuted, fontSize: 10)),
            ],
            if (_uploadUrl != null && _selectedFile == null) ...[
               const SizedBox(height: 12),
               TextButton.icon(
                 onPressed: () => launchUrl(Uri.parse(_uploadUrl!)),
                 icon: const Icon(Icons.open_in_new, size: 14),
                 label: const Text('View Current File', style: TextStyle(fontSize: 12)),
               ),
            ],
          ],
        ),
  ...
  Widget _buildLockedAlert() {
    return Container(
      ...
      child: Column(
        children: [
          const Icon(Icons.lock_person_outlined, color: AppColors.success, size: 48),
          const SizedBox(height: 16),
          const Text('Submission Approved', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 18)),
          const SizedBox(height: 8),
          const Text('This report has been reviewed and marked as completed. You can no longer edit this submission.', textAlign: TextAlign.center, style: TextStyle(color: Colors.white70, fontSize: 13)),
          const SizedBox(height: 20),
          if (_uploadUrl != null) 
            ElevatedButton.icon(
              onPressed: () => launchUrl(Uri.parse(_uploadUrl!)),
              icon: const Icon(Icons.remove_red_eye_outlined),
              label: const Text('View Approved Document'),
              style: ElevatedButton.styleFrom(backgroundColor: AppColors.success.withAlpha(50), foregroundColor: Colors.white),
            )
        ],
      ),
    );
